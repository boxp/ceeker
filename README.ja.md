...
Hook CLI コマンド（`ceeker hook ...`）は IO 操作（状態永続化・古いセッション整理）をバックグラウンドスレッドで非同期実行します。これにより、ファイルロック遅延や tmux クエリの遅延が呼び出し元エージェントをブロックすることを防ぎます。

**動作:**

- ペイロード解析・正規化は同期的に実行（高速）
- 状態永続化（`sessions.edn` 書き込み）および古いセッション整理はバックグラウンドで実行
- バックグラウンド処理は **2秒のタイムアウト** を持ち、超過した場合は待機せず終了
- バックグラウンドエラーは stderr にログ出力されますが、呼び出し元エージェントには **伝播しません**
- hook の失敗や遅延によってエージェントプロセスがブロックされることはありません
